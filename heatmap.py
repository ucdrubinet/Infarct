...
def main():
    ...
    parser.add_argument("--model_path", type=str, default='./r18.pt', help="Saved model")
    parser.add_argument("--smaller_model_path", type=str, default='./r18_small.pt', help="If multiple FOV, specify path to second saved model")
    parser.add_argument("--wsi_dir", type=str, default='inf_tiles/', help="Directory to retrieve the patches for heatmaps")
    ...
    print(f"Tile size: {args.img_size}")
    ...
    if not os.path.exists(SAVE_NP_DIR):
        print("Heatmap folder you provided us does not exist, being created now...")
    ...
    checkpoint = torch.load(MODEL_SEG_DIR,map_location=torch.device('cuda:'+str(args.gpu)))

    if isMultiFOV:
        seg_model_small = seg_model
        checkpoint_small = torch.load(MODEL_SEG_DIR_SMALL,map_location=torch.device('cuda:'+str(args.gpu)))
    ...
    for filename in filenames[:]:
        ...
        cols = [int(image.split('/')[-1].split('.')[0].split('_')[-1]) for image in imgs]
        col_nums = max(cols) +1    
        
        print("Row nums = ", row_nums)
        print("Col nums = ", col_nums)

        # Initialize outputs accordingly:
        heatmap_res = patch_size // stride
        
        print("heatmap resolution = ", heatmap_res*row_nums, " x ", heatmap_res*col_nums)
