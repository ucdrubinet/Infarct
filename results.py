...
def run(MASK_DIR,WSI_DIR,NP_MAP_DIR,HEATMAP_DIR,gtdf,scale_parameters,distance_threshold,area_thresh_par,threshold_area,kernelsz,predconf,rule,isMultiFOV,micrometer_diameter,n_rule,conf_rule):
    tp8_total = 0 
    fp8_total = 0 
    ...
    for heatmap_pt in sorted(os.listdir(HEATMAP_DIR)):
        ...
        ground_truth_image_path =  MASK_DIR+caseID+'.npy'
...
def main():
    ...
    parser.add_argument("--save_np_dir", type=str, default='./Infseg/', help="Directory for heatmap .npy files")
    parser.add_argument("--heatmap_dir", type=str, default='./outputs/', help="Directory for heatmap images")
    ...
    parser.add_argument("--WSI_level_threshold", type=int, default=5000, help="Threshold (in pixels) for detection to be turned into WSI level label")
    parser.add_argument("--wsi_dir", type=str, default='inf_tiles/', help="Directory of Whole Slide Images")
    ...
    if not os.path.exists(HEATMAP_DIR):
        print("Heatmap image folder you provided us does not exist, being created now...")
        os.makedirs(HEATMAP_DIR)

    gtdf = pd.read_csv('gt_plus_wmr_pvs.csv')
    run(MASK_DIR,WSI_DIR,NP_MAP_DIR,HEATMAP_DIR,gtdf,scale_parameters,distance_threshold,area_thresh_par,threshold_area,kernelsz,predconf,rule,isMultiFOV,micrometer_diameter,n_rule,conf_rule)
